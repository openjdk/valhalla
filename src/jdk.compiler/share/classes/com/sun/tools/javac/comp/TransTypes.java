...
//      System.err.println("retype " + tree + " to " + erasedType);//DEBUG
     ...
     *  @param originalParameterTypes     The list of target original parameter types.
     *  @param varargsElement             The erasure of the varargs element type,
     *                                    or null if translating a non-varargs invocation
     */
    ...
    <T extends JCTree> List<T> translateArgs(List<T> _args,
                                           ...
                                           List<Type> originalParameterTypes,
                                           Type varargsElement,
                                           Type originalVarargsElement) {
        if (erasedParameterTypes.isEmpty()) return _args;
        List<T> args = _args;
        while (erasedParameterTypes.tail.nonEmpty()) {
            if (!originalParameterTypes.isEmpty() && originalParameterTypes.head != null) {
                if (types.isNonNullable(originalParameterTypes.head) && !types.isNonNullable(args.head.type)) {
            ...
            if (!originalParameterTypes.isEmpty()) {
                originalParameterTypes = originalParameterTypes.tail;
            }
        }
 ...
 *************************************************************************/
   ...
   public void visitIf(JCIf tree) {
    ...
    public void visitApply(JCMethodInvocation tree) {
        ...
        tree.args = translateArgs(tree.args, argtypes, meth.type.getParameterTypes(), tree.varargsElement, originalVarargsElement);
