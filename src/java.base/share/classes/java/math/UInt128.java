...
/**
 * Immutable unsigned {@link #SIZE}-bit integers ({@link #SIZE} = 128).
 * <p>
...
public final class UInt128 implements Comparable<UInt128> {
     ...
     *      Or if the characters in the portion are neither ASCII, nor valid digits,
     *      nor separators.
    ...
    public static UInt128 parse(CharSequence s, int begin, int end,
        ...
        if (2 > radix || radix > 36) {
            throw new IllegalArgumentException("radix not in [2, 36]");
        }
     ...
     *
     * @param x the argument.
     * @param sh nly the {@link #LOG2_SIZE} least significant bits are relevant.
     * @return the result described above.
     */
    public static UInt128 shiftRight(UInt128 x, int sh) {
        sh &= SIZE - 1;  // only the LOG2_SIZE least significant bits count
        return sh >= 64
                ? new UInt128(x.c1 >>> sh)
