...
public abstract value class IntVector extends AbstractVector<Integer> {
   ...
   @ForceInline
    ...
    static IntVector selectFromTwoVectorHelper(Vector<Integer> indexes, Vector<Integer> src1, Vector<Integer> src2) {
        ...
        VectorPayloadMF vecPayload1 = ((IntVector)indexes).vec();
        VectorPayloadMF vecPayload2 = ((IntVector)src1).vec();
        VectorPayloadMF vecPayload3 = ((IntVector)src2).vec();
        AbstractSpecies<Integer> vsp = ((IntVector)src1).vspecies();
        VectorPayloadMF tpayload = U.makePrivateBuffer(vecPayload2);
        long vOffset = tpayload.multiFieldOffset();
        for (int i = 0; i < vlen; i++) {
            int wrapped_index = VectorIntrinsics.wrapToRange((int)U.getInt(vecPayload1, vOffset + i * Integer.BYTES), 2 * vlen);
            U.putInt(tpayload, vOffset + i * Integer.BYTES, wrapped_index >= vlen ?
                        U.getInt(vecPayload3, vOffset + (wrapped_index  - vlen) * Integer.BYTES) :
                        U.getInt(vecPayload2, vOffset + wrapped_index * Integer.BYTES));
        }
        tpayload = U.finishPrivateBuffer(tpayload);
        return ((IntVector)(vsp.dummyVectorMF())).vectorFactory(tpayload);
    }
    ...
    private static BinaryOperation<IntVector, VectorMask<Integer>> binaryOperationsMF(int opc_) {
        switch (opc_) {
            ...
            case VECTOR_OP_UMAX: return (v0, v1, vm) ->
                    v0.bOpMF(v1, vm, (i, a, b) -> (int)VectorMath.maxUnsigned(a, b));
            case VECTOR_OP_UMIN: return (v0, v1, vm) ->
                    v0.bOpMF(v1, vm, (i, a, b) -> (int)VectorMath.minUnsigned(a, b));
            case VECTOR_OP_SADD: return (v0, v1, vm) ->
                    v0.bOpMF(v1, vm, (i, a, b) -> (int)(VectorMath.addSaturating(a, b)));
            case VECTOR_OP_SSUB: return (v0, v1, vm) ->
                    v0.bOpMF(v1, vm, (i, a, b) -> (int)(VectorMath.subSaturating(a, b)));
            case VECTOR_OP_SUADD: return (v0, v1, vm) ->
                    v0.bOpMF(v1, vm, (i, a, b) -> (int)(VectorMath.addSaturatingUnsigned(a, b)));
            case VECTOR_OP_SUSUB: return (v0, v1, vm) ->
                    v0.bOpMF(v1, vm, (i, a, b) -> (int)(VectorMath.subSaturatingUnsigned(a, b)));
    ...
    private static ReductionOperation<IntVector, VectorMask<Integer>> reductionOperations(int opc_) {
        switch (opc_) {
            ...
            case VECTOR_OP_UMIN: return (v, m) ->
                    toBits(v.rOpMF(MAX_OR_INF, m, (i, a, b) -> (int) VectorMath.minUnsigned(a, b)));
            case VECTOR_OP_UMAX: return (v, m) ->
                    toBits(v.rOpMF(MIN_OR_INF, m, (i, a, b) -> (int) VectorMath.maxUnsigned(a, b)));
            case VECTOR_OP_SUADD: return (v, m) ->
                    toBits(v.rOpMF((int)0, m, (i, a, b) -> (int) VectorMath.addSaturatingUnsigned(a, b)));
