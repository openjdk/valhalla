...
InlineTypeNode* PhiNode::push_inline_types_down(PhaseGVN* phase, bool can_reshape, ciInlineKlass* inline_klass) {
  if (can_reshape) {
    ...
    ResourceMark rm;
    Unique_Node_List wq, subgraph_to_clone;
    Node_List clones;
    ...
    DEBUG_ONLY(uint init_nodes = wq.size());
    ...
    for (uint i = 0; i < wq.size(); ++i) {
      ...
      if (n->is_Phi()) {
        ...
        wq.push(buf);
        subgraph_to_clone.push(n);
      }
    }
    // Find the subgraph that must be cloned by following uses.
    for (uint i = 0; i < subgraph_to_clone.size(); ++i) {
      Node* n = subgraph_to_clone.at(i);
      for (DUIterator_Fast imax, i = n->fast_outs(imax); i < imax; i++) {
        Node* u = n->fast_out(i);
        if (wq.member(u)) {
          subgraph_to_clone.push(u);
        }
      }
    }
    // clone the nodes of the subgraph
    for (uint i = 0; i < subgraph_to_clone.size(); ++i) {
      Node* n = subgraph_to_clone.at(i);
    ...
    // Update edges of the cloned sugraph
    for (uint i = 0; i < subgraph_to_clone.size(); ++i) {
      Node* n = subgraph_to_clone.at(i);
...
Node* PhiNode::try_push_inline_types_down(PhaseGVN* phase, const bool can_reshape) {
  ...
  if (push_inline_type_down.can_do_it()) {
    assert(can_push_inline_types_down(phase, can_reshape, inline_klass), "");
