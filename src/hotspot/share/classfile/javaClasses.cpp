...
void java_lang_Class::fixup_mirror(Klass* k, TRAPS) {
  ...
  if (!k->in_aot_cache()) {
    if (k->is_instance_klass()) {
      ...
      Array<u1>* new_fis = FieldInfoStream::create_FieldInfoStream(fields, java_fields, injected_fields, k->class_loader_data(),
                                                                   ik->multifield_info(), CHECK);
      ...
      Array<u1>* search_table = FieldInfoStream::create_search_table(ik->constants(), new_fis, k->class_loader_data(),
                                                                     ik->multifield_info(), CHECK);
      ik->set_fieldinfo_search_table(search_table);
      MetadataFactory::free_array<u1>(k->class_loader_data(), old_table);

      DEBUG_ONLY(FieldInfoStream::validate_search_table(ik->constants(), new_fis, search_table, ik->multifield_info()));
    }
  }
