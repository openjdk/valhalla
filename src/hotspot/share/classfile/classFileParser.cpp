...
static void parse_annotations(const ConstantPool* const cp,
  ...
  enum {  // initial annotation layout
    ...
    s_size = 9,
    b_tag_val = 'B',  // payload is byte
    b_con_off = 7,
    b_size = 9,
    z_tag_val = 'Z',  // payload is boolean
  ...
  *java_fields_count_ptr = *java_fields_count_ptr + inj_multifields;
  if (num_injected != 0) {
    for (int n = 0; n < num_injected; n++) {
      ...
      assert(idx == field_index, "Must be");
  ...
  if (_need_verify && length > 1) {
    ...
    for (int i = 0; i < _temp_field_info->length(); i++) {
      NameSigHash name_and_sig(_temp_field_info->adr_at(i)->name(_cp, _multifield_info),
...
void ClassFileParser::apply_parsed_class_metadata(
  ...
  DEBUG_ONLY(FieldInfoStream::validate_search_table(_cp, _fieldinfo_stream, _fieldinfo_search_table, _multifield_info));
...
void ClassFileParser::post_process_parsed_stream(const ClassFileStream* const stream,
    ...
    for (GrowableArrayIterator<FieldInfo> it = _temp_field_info->begin(); it != _temp_field_info->end(); ++it) {
      ...
      if (fieldinfo.field_flags().is_null_free_inline_type()) {
        ...
        if (name != _class_name && is_class_in_loadable_descriptors_attribute(sig)) {
          if (!ClassFileParser::is_jdk_internal_class(name)) {
            log_info(class, preload)("Preloading of class %s during loading of class %s. "
  ...
  FieldLayoutBuilder lb(class_name(), loader_data(), super_klass(), _cp, /*_fields*/ _temp_field_info,
      ...
      _must_be_atomic, _layout_info, _inline_layout_info_array, _multifield_info);
  lb.build_layout();
  ...
  _fieldinfo_stream =
    FieldInfoStream::create_FieldInfoStream(_temp_field_info, _java_fields_count,
                                            injected_fields_count, loader_data(), _multifield_info, CHECK);
  _fieldinfo_search_table = FieldInfoStream::create_search_table(_cp, _fieldinfo_stream, _loader_data, _multifield_info, CHECK);
  _fields_status =
