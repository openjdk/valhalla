...
Array<u1>* FieldInfoStream::create_FieldInfoStream(GrowableArray<FieldInfo>* fields, int java_fields, int injected_fields,
                                                   ClassLoaderData* loader_data, const Array<MultiFieldInfo>* multifield_info, TRAPS) {
...
Array<u1>* FieldInfoStream::create_search_table(ConstantPool* cp, const Array<u1>* fis, ClassLoaderData* loader_data,
                                                const Array<MultiFieldInfo>* multifield_info, TRAPS) {
  ...
  for (int i = 0; i < java_fields; ++i) {
    ...
    positions[i]._name = fi.name(cp, multifield_info);
...
public:
  FieldInfoComparator(const FieldInfoReader* reader, ConstantPool* cp, const Symbol* name, const Symbol* signature,
                      const Array<MultiFieldInfo>* multifield_info):
    _reader(reader), _cp(cp), _name(name), _signature(signature), _multifield_info(multifield_info) {}

  Symbol* get_name(FieldInfoReader r2, u2 name_index) {
    u4 offset = r2.next_uint();
    AccessFlags access_flags = AccessFlags(checked_cast<u2>(r2.next_uint()));
    FieldInfo::FieldFlags field_flags = FieldInfo::FieldFlags(r2.next_uint());
    Symbol* name = nullptr;
    if (field_flags.is_multifield()) {
      name = _multifield_info->at(name_index).name();
    } else {
      name = _cp->symbol_at(name_index);
    }
    return name;
  }
  ...
  int compare_to(uint32_t position) override {
    ...
    Symbol* mid_name = get_name(r2, name_index);
...
void FieldInfoStream::validate_search_table(ConstantPool* cp, const Array<u1>* fis, const Array<u1>* search_table,
  ...
  FieldInfoComparator comparator(&reader, cp, nullptr, nullptr, multifield_info);
  ...
  while (reader.has_next()) {
    ...
    int index = r2.search_table_lookup(search_table, fi.name(cp, multifield_info), fi.signature(cp), cp, java_fields, multifield_info);
...
void FieldInfoStream::print_search_table(outputStream* st, ConstantPool* cp, const Array<u1>* fis, const Array<u1>* search_table,
  ...
  auto printer = [&] (size_t offset, uint32_t position, uint32_t index) {
    ...
    u4 skip_word = reader.next_uint();
    AccessFlags access_flags = AccessFlags(checked_cast<u2>(reader.next_uint()));
    FieldInfo::FieldFlags field_flags = FieldInfo::FieldFlags(reader.next_uint());
    Symbol* name = nullptr;
    if (field_flags.is_multifield()) {
      name = multifield_info->at(name_index).name();
    } else {
...
int FieldInfoReader::search_table_lookup(const Array<u1>* search_table, const Symbol* name, const Symbol* signature,
                                         ConstantPool* cp, int java_fields, const Array<MultiFieldInfo>* multifield_info) {
  assert(java_fields >= 0, "must be");
  if (java_fields == 0) {
    return -1;
  }
  FieldInfoComparator comp(this, cp, name, signature, multifield_info);
