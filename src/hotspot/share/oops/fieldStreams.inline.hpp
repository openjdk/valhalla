...
FieldStreamBase::FieldStreamBase(const Array<u1>* fieldinfo_stream, ConstantPool* constants, Array<MultiFieldInfo>* multifield_info) :
        _fieldinfo_stream(fieldinfo_stream),
        _reader(FieldInfoReader(_fieldinfo_stream)),
        _constants(constantPoolHandle(Thread::current(), constants)),
        _multifield_info(multifield_info),
...
inline bool JavaFieldStream::lookup(const Symbol* name, const Symbol* signature) {
  if (_search_table != nullptr) {
    int index = _reader.search_table_lookup(_search_table, name, signature, _constants(), _limit, _multifield_info);
    if (index >= 0) {
