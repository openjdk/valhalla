...
void InstanceKlass::deallocate_contents(ClassLoaderData* loader_data) {
  ...
  if (_multifield_info != NULL && !_multifield_info->in_aot_cache()) {
...
bool InstanceKlass::find_local_field(Symbol* name, Symbol* sig, fieldDescriptor* fd) const {
  ...
  if (fs.lookup(name, sig)) {
    ...
    fd->reinitialize(const_cast<InstanceKlass*>(this), fs.to_FieldInfo(), _multifield_info);
    return true;
  }
  return false;
}
...
void InstanceKlass::print_nonstatic_fields(FieldClosure* cl) {
  ...
  if (length > 0) {
    ...
    for (int i = 0; i < length; i++) {
      fd.reinitialize(this, fields_sorted.at(i), _multifield_info);
...
void InstanceKlass::remove_unshareable_info() {
  ...
  DEBUG_ONLY(FieldInfoStream::validate_search_table(_constants, _fieldinfo_stream, _fieldinfo_search_table, _multifield_info));
}
...
void InstanceKlass::print_on(outputStream* st) const {
  ...
  if (fieldinfo_search_table() != nullptr) {
    ...
    FieldInfoStream::print_search_table(st, _constants, _fieldinfo_stream, _fieldinfo_search_table, _multifield_info);
  }
}
