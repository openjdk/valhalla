...
inline void RawAccessBarrier<decorators>::value_copy(void* src, void* dst, InlineKlass* md) {
  assert(is_aligned(src, md->get_alignment()) && is_aligned(dst, md->get_alignment()), "Unaligned value_copy");
  AccessInternal::value_copy_internal(src, dst, static_cast<size_t>(md->get_payload_size_in_bytes()));
}
