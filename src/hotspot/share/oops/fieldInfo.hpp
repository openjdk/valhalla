...
class FieldInfo {
 ...
 public:
  ...
  inline Symbol* name(ConstantPool* cp, const Array<MultiFieldInfo>* multifield_info = nullptr) const;

  inline Symbol* signature(ConstantPool* cp) const;

  inline Symbol* lookup_symbol(int symbol_index) const;

  Symbol* get_multifield_name(const Array<MultiFieldInfo>* multifield_info) const;

  inline u2 multifield_base(const Array<MultiFieldInfo>* multifield_info) const;

  inline jbyte multifield_index(const Array<MultiFieldInfo>* multifield_info) const;
...
public:
  ...
  int search_table_lookup(const Array<u1>* search_table, const Symbol* name, const Symbol* signature, ConstantPool* cp,
                          int java_fields, const Array<MultiFieldInfo>* multifield_info);
...
class FieldInfoStream : AllStatic {
 ...
 public:
  ...
  static Array<u1>* create_FieldInfoStream(GrowableArray<FieldInfo>* fields, int java_fields, int injected_fields,
                                           ClassLoaderData* loader_data, const Array<MultiFieldInfo>* multifields_info, TRAPS);
  static Array<u1>* create_search_table(ConstantPool* cp, const Array<u1>* fis, ClassLoaderData* loader_data,
                                        const Array<MultiFieldInfo>* multifields_info, TRAPS);
  ...
  DEBUG_ONLY(static void validate_search_table(ConstantPool* cp, const Array<u1>* fis, const Array<u1>* search_table,
                                               const Array<MultiFieldInfo>* multifields_info);)
