...
JNI_ENTRY(jfieldID, jni_FromReflectedField(JNIEnv *env, jobject field))
  ...
  intptr_t offset = InstanceKlass::cast(k1)->field_offset( slot );
  bool is_allocated_inline = InstanceKlass::cast(k1)->field_is_allocated_inline(slot);
  assert(InstanceKlass::cast(k1)->contains_field_offset(offset), "stay within object");
  ret = jfieldIDWorkaround::to_instance_jfieldID(k1, offset, is_allocated_inline);
  return ret;
JNI_END
...
JNI_ENTRY(jfieldID, jni_GetFieldID(JNIEnv *env, jclass clazz,
  ...
  // It may also have hash bits for k, if VerifyJNIFields is turned on.
  ret = jfieldIDWorkaround::to_instance_jfieldID(k, fd.offset(), fd.is_allocated_inline());
  return ret;
JNI_END
...
JNI_ENTRY(jobject, jni_GetObjectField(JNIEnv *env, jobject obj, jfieldID fieldID))
  ...
  if (!jfieldIDWorkaround::is_allocated_inline_jfieldID(fieldID)) {
    ...
    res = field_vklass->read_field_allocated_inline(o, ik->field_offset(fd.index()), CHECK_NULL);
  }
...
JNI_ENTRY_NO_PRESERVE(void, jni_SetObjectField(JNIEnv *env, jobject obj, jfieldID fieldID, jobject value))
  ...
  if (!jfieldIDWorkaround::is_allocated_inline_jfieldID(fieldID)) {
    ...
    vklass->write_field_allocated_inline(o, offset, v, CHECK);
  }
...
JNI_ENTRY(jsize, jni_GetFieldOffsetInFlattenedLayout(JNIEnv* env, jclass clazz, const char *name, const char *signature, jboolean* is_allocated_inline))
  ...
  if (is_allocated_inline != NULL) {
    *is_allocated_inline = fd.is_allocated_inline();
  }
  return (jsize)offset;
JNI_END
...
JNI_ENTRY(jobject, jni_CreateSubElementSelector(JNIEnv* env, jarray array))
  ...
  jdk_internal_vm_jni_SubElementSelector::setIsAllocatedInline(ses_h(), true);   // by definition, top element of a flattened array is allocated inline
...
JNI_ENTRY(jobject, jni_GetSubElementSelector(JNIEnv* env, jobject selector, jfieldID fieldID))
  ...
  jboolean is_allocated_inline = jdk_internal_vm_jni_SubElementSelector::getIsAllocatedInline(slct);
  if (!is_allocated_inline) {
    THROW_MSG_NULL(vmSymbols::java_lang_IllegalArgumentException(), "SubElement is not allocated inline");
  }
  ...
  jdk_internal_vm_jni_SubElementSelector::setIsAllocatedInline(res_h(), fd.is_allocated_inline());
...
JNI_ENTRY(jobject, jni_GetObjectSubElement(JNIEnv* env, jarray array, jobject selector, int index))
  ...
  oop res = NULL;
  if (!jdk_internal_vm_jni_SubElementSelector::getIsAllocatedInline(slct)) {
