...
public class ConcurrentClassLoadingTest {
  ...
  public static void doTest(int n) {
    ...
    var cl = new BigClassTreeClassLoader(DEPTH, fields);
    Thread[] threads = new Thread[n];
    // Spawn all the threads with their respective worker classes.
    for (int i = 0; i < n; i++) {
      Thread thread = new Thread(new Worker(barrier, cl));
      threads[i] = thread;
      thread.start();
    }
  ...
  public static class Worker implements Runnable {
    ...
    public Worker(CyclicBarrier barrier, ClassLoader loader) {
    ...
    public void run() {
      try {
        ...
        Class<?> workerClass = Class.forName("Gen" + (DEPTH - 1), false, loader);
        Object worker = workerClass.getDeclaredConstructor().newInstance();
      } catch(InterruptedException | BrokenBarrierException e) {
        throw new IllegalStateException("test setup: waiting for all threads saw error", e);
