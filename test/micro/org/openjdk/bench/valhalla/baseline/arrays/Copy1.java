...
public class Copy1 extends SizedBase {
    ...
    @Setup
    public void setup() {
        srcPrimitive = Utils.fill(new int[size]);
        dstPrimitive = new int[size];
        srcReference = Utils.fill(new Ref1[size]);
        dstReference = new Ref1[size];
        srcCovariance = Utils.fill(new Ref1[size]);
        dstCovariance = new Ref1[size];
    }
    ...
    public void reference() {
        Ref1[] src = srcReference;
        Ref1[] dst = dstReference;
        for (int i = 0; i < size; i++) {
            dst[i] = src[i];
        }
    }
