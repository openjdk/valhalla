...
public class Ackermann2 extends AckermannBase {
    ...
    private static Ref2 ack_reference(Ref2 x, Ref2 y) {
        return x.reduce() == 0 ?
                y.inc() :
                (y.reduce() == 0 ?
                        ack_reference(x.dec(), new Ref2(0, 1)) :
    ...
    public int reference() {
        return ack_reference(new Ref2(0, X1), new Ref2(0, Y1)).reduce()
             + ack_reference(new Ref2(0, X2), new Ref2(0, Y2)).reduce()
             + ack_reference(new Ref2(0, X2), new Ref2(0, Y3)).reduce();
    }
