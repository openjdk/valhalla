...
@State(Scope.Thread)
public class IsCmpBranch2 extends ACmpBase {

    public static final int SIZE = 1024;

    Val2[] value1_00, value2_00;
    ...
    public static Val2[] populateValues1() {
        Val2[] values = new Val2[SIZE];
        for(int i=0; i< SIZE; i++) {
            values[i] = new Val2(0, i);
        }
        return values;
    }

    public static Val2[] populateValues2(Val2[] base, int eq) {
        Val2[] values2 = new Val2[base.length];
        for(int i=0; i< SIZE; i++) {
            values2[i] = new Val2(1, SIZE + i);
        }
        if (eq == 0) {
            // nothing to do
        } else if (eq >= 100) {
    ...
    private static int value_comparison(Val2[] objects1, Val2[] objects2) {
        int s = 0;
        for (int i = 0; i < SIZE; i++) {
