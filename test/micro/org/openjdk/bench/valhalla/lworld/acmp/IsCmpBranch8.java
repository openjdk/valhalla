...
@State(Scope.Thread)
public class IsCmpBranch8 extends ACmpBase {

    public static final int SIZE = 1024;

    Val8[] value1_00, value2_00;
    ...
    public static Val8[] populateValues1() {
        Val8[] values = new Val8[SIZE];
        for(int i=0; i< SIZE; i++) {
            values[i] = new Val8(0, 0, 0, 0, 0, 0, 0, i);
        }
        return values;
    }

    public static Val8[] populateValues2(Val8[] base, int eq) {
        Val8[] values2 = new Val8[base.length];
        for(int i=0; i< SIZE; i++) {
            values2[i] = new Val8(0, 0, 0, 0, 0, 0, 1, SIZE + i);
        }
        if (eq == 0) {
            // nothing to do
        } else if (eq >= 100) {
    ...
    private static int value_comparison(Val8[] objects1, Val8[] objects2) {
        int s = 0;
        for (int i = 0; i < SIZE; i++) {
