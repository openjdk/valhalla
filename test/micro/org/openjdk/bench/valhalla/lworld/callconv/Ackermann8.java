...
public class Ackermann8 extends AckermannBase {
    ...
    private static Val8 ack_value(Val8 x, Val8 y) {
        return x.reduce() == 0 ?
                y.inc() :
                (y.reduce() == 0 ?
                        ack_value(x.dec(), new Val8(0, 0, 0, 0, 0, 0, 0, 1)) :
    ...
    @OperationsPerInvocation(OPI)
    public int boxed() {
        return ack_boxed(new Val8(0, 0, 0, 0, 0, 0, 0, X1), new Val8(0, 0, 0, 0, 0, 0, 0, Y1)).reduce() +
               ack_boxed(new Val8(0, 0, 0, 0, 0, 0, 0, X2), new Val8(0, 0, 0, 0, 0, 0, 0, Y2)).reduce() +
               ack_boxed(new Val8(0, 0, 0, 0, 0, 0, 0, X3), new Val8(0, 0, 0, 0, 0, 0, 0, Y3)).reduce();
    }
