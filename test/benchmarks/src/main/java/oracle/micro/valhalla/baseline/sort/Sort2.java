...
public class Sort2 extends SortBase {
    ...
    public Box2[] makeRandomArray() {
        Box2[] arr = new Box2[size];
        for (int i = 0; i < size; i++) {
            arr[i] = new Box2 (ThreadLocalRandom.current().nextInt(),ThreadLocalRandom.current().nextInt());
        }
        return arr;
    }
    ...
    @Benchmark
    public Box2[] quicksort() {
        Box2[] toSort = makeRandomArray();
    ...
    public Box2[] mergeSort() {
        Box2[] toSort = makeRandomArray();
        Box2[] aux = new Box2[toSort.length];
        System.arraycopy(toSort, 0, aux, 0 , toSort.length);
        mergeSort(aux, toSort, 0, toSort.length, 0);
        return toSort;
    }

    private static int compare(Box2 a, Box2 b) {
        int c0 = Integer.compare(a.f0, b.f0);
        return c0!=0 ? c0: Integer.compare(a.f0, b.f0);

    }

    //##########3 dual pivot quick sort##############################

    private static void sort1(Box2 x[], int off, int len) {
     ...
     * Swaps x[a] with x[b].
     */
    private static void swap(Box2 x[], int a, int b) {
        Box2 t = x[a];
        x[a] = x[b];
        x[b] = t;
    }
     ...
     */
    private static void vecswap(Box2 x[], int a, int b, int n) {
     ...
     */
    private static int med3(Box2 x[], int a, int b, int c) {
